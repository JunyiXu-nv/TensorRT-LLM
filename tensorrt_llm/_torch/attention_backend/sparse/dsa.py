...
class Indexer(nn.Module):
    ...
    def pre_indexer_proj(
        ...
        # When the upstream allreduce fused the next layer's kv_a_proj NVFP4
        # quantize, hidden_states is an Fp4QuantizedTensor that also carries
        # the BF16 post-RMSNorm value. Use that BF16 view here -- the indexer
        # weight is BF16 and cublas_mm needs a float input.
        if isinstance(hidden_states, Fp4QuantizedTensor):
            assert hidden_states.bf16_hidden_states is not None, (
                ...
                "the producer fusion must use RESIDUAL_RMS_NORM_OUT_QUANT_NVFP4")

...
class PyExecutor:
    ...
    def __init__(
        ...
        # an exception (e.g. KV cache OOM), it is stashed here so callers
        # parked in _await_single_response wake on shutdown and re-raise
        # the original error instead of hanging on response_cv forever.
    ...
    def _event_loop_wrapper(self):
        try:
            ...
            # Stash the original error so _await_single_response can
            # surface it instead of letting callers hang. We do NOT call
            # _handle_errors / _enqueue_responses here: they trigger
    ...
    def _await_single_response(
        ...
        with self.response_cv:
            ...
            if self.is_shutdown:
                ...
                # instead of an empty list / KeyError.
                ...
                raise RuntimeError(
                    ...
                    f"for request {id}.")
            # Timed out without shutdown — preserve legacy behaviour.
            return []

...
the results back to the model within a single exchange. Anthropic Messages
calls it ``web_search_20250305`` and the Responses API calls it
``web_search``/``web_search_preview``, but the work behind both is the same,
so it lives here rather than in either endpoint's adapter.
...
The older ``TRTLLM_ANTHROPIC_WEB_SEARCH*`` names are still honoured, since
deployments and launch scripts already set them; the neutral name wins when
both are present.
"""
...
def _env(suffix: str) -> Optional[str]:
    """Read a setting, preferring the neutral name over the legacy one.

    The feature shipped as Anthropic-only, so existing deployments and launch
    scripts set TRTLLM_ANTHROPIC_WEB_SEARCH*. Those keep working; renaming
    the module must not silently turn a configured provider off.
    """
    value = os.environ.get(f"TRTLLM_WEB_SEARCH{suffix}")
    if value is None:
        value = os.environ.get(f"TRTLLM_ANTHROPIC_WEB_SEARCH{suffix}")
    return value
...
def _env_float(suffix: str, default: float) -> float:
    ...
    try:
        ...
        logger.warning("web search %s=%r is not a number; using %s", suffix, raw,
                       default)
        return default
...
def _env_int(suffix: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning("web search %s=%r is not an integer; using %s", suffix,
                       raw, default)
        return default
...
class WebSearchToolSpec:
    ...
    Every API spells the tool differently - Anthropic sends
    ``web_search_20250305`` with ``max_uses``, the Responses API sends
    ``web_search``/``web_search_preview`` - but all any of them contribute is
    a name and some limits. Reducing a request's tool to this keeps the rest
    of the module from importing either endpoint's types.
    """
...
class WebSearchSession:
    ...
    def budget_notice(self) -> str:
        return (f"The web search budget for this request is used up "

...
std::tuple<th::Tensor, th::Tensor> indexer_k_cache_gather_op(
    th::Tensor const& k_cache, th::Tensor const& slot_mapping_fp8, th::Tensor const& slot_mapping_scale)
{
    ...
    auto num_tokens = static_cast<int32_t>(slot_mapping_fp8.size(0));
    TORCH_CHECK(
        slot_mapping_scale.size(0) == num_tokens, "slot_mapping_scale length must equal slot_mapping_fp8 length");
    ...
    // DeepSeek-V3.2 constants
    constexpr int32_t head_dim = 128;
    constexpr int32_t scale_size = 4; // float32 = 4 bytes
    ...
    auto options_u8 = th::TensorOptions().dtype(torch::kUInt8).device(k_cache.device());
    th::Tensor k_fp8_out = th::empty({num_tokens, head_dim}, options_u8);
    th::Tensor k_scale_out = th::empty({num_tokens, scale_size}, options_u8);

    auto stream = at::cuda::getCurrentCUDAStream(k_cache.get_device());

    tk::invokeIndexerKCacheGather(k_fp8_out.data_ptr<uint8_t>(), k_scale_out.data_ptr<uint8_t>(),
...
{
    m.def(
        "indexer_k_cache_gather_op(Tensor k_cache, "
        "Tensor slot_mapping_fp8, Tensor slot_mapping_scale) -> (Tensor, Tensor)");
}

...
from tensorrt_llm.serve.web_search import (SearchOutcome, WebSearchConfig,
                                           WebSearchResult, WebSearchSession,
                                           WebSearchToolSpec,
...
def os_environ_names():
    ...
    return [
        ...
        if n.startswith("TRTLLM_WEB_SEARCH")
        or n.startswith("TRTLLM_ANTHROPIC_WEB_SEARCH")
    ]
...
def test_legacy_env_still_works(monkeypatch):
    """Deployments and launch scripts already set the Anthropic-era name."""
    monkeypatch.setenv("TRTLLM_ANTHROPIC_WEB_SEARCH", "wikipedia")
    assert load_web_search_config().provider == "wikipedia"


def test_neutral_env_wins_over_legacy(monkeypatch):
    monkeypatch.setenv("TRTLLM_ANTHROPIC_WEB_SEARCH", "mojeek")
    monkeypatch.setenv("TRTLLM_WEB_SEARCH", "wikipedia")
    assert load_web_search_config().provider == "wikipedia"


def test_numeric_settings_honour_both_names(monkeypatch):
    monkeypatch.setenv("TRTLLM_ANTHROPIC_WEB_SEARCH_MAX_RESULTS", "3")
    assert load_web_search_config().max_results == 3
...
def _session(monkeypatch, results=None, error=None, max_uses=2):
    ...
    monkeypatch.setattr("tensorrt_llm.serve.web_search.run_web_search",
                        fake_run)
    return session
...
def test_results_render_for_the_model():
    outcome = SearchOutcome(
        query="trtllm",
        results=[WebSearchResult("https://x.example", "Title", "Snippet")])

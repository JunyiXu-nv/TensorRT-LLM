...
from tensorrt_llm._torch.pyexecutor.scheduler.adp_router import KVCacheAwareADPRouter, RankState
...
def _make_request_item(
    req_id, num_tokens=100, client_id=None, target_dp_rank=None, disagg_request_id=None
):
    item = MagicMock()
    item.id = req_id
    ...
    item.request.disagg_request_id = disagg_request_id
...
def test_trace_off_by_default():
    ...
    router.route_requests(
        _states((0, 0), (0, 0)), [_make_request_item(1)], max_num_active_requests=10
    )
...
def test_scored_decision_records_every_candidate_rank():
    ...
    router.route_requests(
        _states((0, 0), (0, 0)),
        [_make_request_item(1, num_tokens=100, client_id=4242, disagg_request_id=99)],
    ...
    # client_id is the serve-edge id the request trace records; req_id is
    # the engine-local queue id.
    assert decision["client_id"] == 4242
    # The disaggregated proxy knows the request only by this id.
    assert decision["disagg_request_id"] == 99
...
def test_load_before_is_captured_ahead_of_any_mutation():
    ...
    router.route_requests(
        _states((1, 100), (3, 700)),
        [_make_request_item(1, num_tokens=50), _make_request_item(2, num_tokens=50)],
        max_num_active_requests=10,
    )
...
def test_load_after_is_reconstructible_from_load_before_and_decisions():
    ...
    router.route_requests(
        _states((0, 0), (0, 0)),
        [_make_request_item(1, num_tokens=40), _make_request_item(2, num_tokens=60)],
        max_num_active_requests=10,
    )
...
def test_strict_placement_recorded_with_its_own_phase():
    ...
    router.route_requests(_states((0, 0), (0, 0)), [item], max_num_active_requests=10)
...
def test_idle_batch_produces_no_record():
    ...
    router.route_requests(_states((0, 0), (0, 0)), [], max_num_active_requests=10)
...
def test_tracing_does_not_change_routing():
    ...
    def run(trace_enabled):
        ...
        router._all_ranks_prefix_matches = [{i: (i * 37) % 90 for i in range(12)} for _ in range(4)]
        states = _states((0, 0), (2, 400), (1, 150), (5, 900))
        items = [_make_request_item(i, num_tokens=50 + (i * 13) % 400) for i in range(12)]
        assigned, expected = router.route_requests(states, items, max_num_active_requests=8)
...
def _trace():
    return {
        "load_before": {"num_active_requests": [0, 1], "num_active_tokens": [0.0, 10.0]},
        "decisions": [
            {
                "req_id": 1,
                "client_id": 1,
...
def test_emit_stamps_the_log_iteration():
    sink = _Sink()
    ...
    # decisions fed, which carries the next counter value because
    # profile_step() prints before the increment.
    assert record["iter"] == 8
